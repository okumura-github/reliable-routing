    ...
    def setValues(self, model, variables, optimal_key):
        ...
        if optimal_key != 0:
            ...
            if self.status == grb.GRB.OPTIMAL:
                ...
                # print("set Values clear!")
            else:
                self.variables = None
        else:
            self.variables = None

    def setValues_okumura(self, judge, variables, optimal_key):
        self.optimal_key = optimal_key
        if optimal_key != 0:
            if judge == True:
                self.status = grb.GRB.OPTIMAL
                self.variables = {"x": {}, "y": {}, "b": {}, "z": {}}
                for link, capacity in variables.x.items():
                    self.variables["x"][link] = capacity
                for link, capacity in variables.y.items():
                    self.variables["y"][link] = capacity
                for link, capacity in variables.b.items():
                    self.variables["b"][link] = capacity
                for link, capacity in variables.z.items():
                    self.variables["z"][link] = capacity
            else:
                self.variables = None
