...
current_link_list5 = copy.deepcopy(link_list)
...
while True:
    ...
    #         # ルート計算
    #         actual_holding_time = traffic_item.CalcRoute(solution, RouteCalcType.ExpectedCapacityGuarantee, MAX_ROUTE,
    ...
    for active_traffic_item in active_traffic_list2[:]:
        # expected_bandwidth = active_traffic_item.traffic.bandwidth * active_traffic_item.traffic.quality  # 帯域幅期待値
        expected_bandwidth = active_traffic_item.traffic.bandwidth * 1
        total_bandwidth = 0
        for route in active_traffic_item.routes[:]:
            for route_link_key, route_link_item in route.items():
                total_bandwidth += route_link_item.bandwidth
    ...
    for active_traffic in active_traffic_list2[:]:
        if active_traffic.end_time <= time:
            # 回線使用終了
            # expected_bandwidth = active_traffic.traffic.bandwidth * active_traffic.traffic.quality
            expected_bandwidth = active_traffic.traffic.bandwidth * 1
            total_bandwidth = 0
            for route in active_traffic.routes:
                added_total_bandwidth = False
                for used_link_key, used_link_item in route.items():
    ...
    if len(traffic_list) > 0:
        for traffic_item in traffic_list[0]:
            ...
            # ルート計算
            traffic_item.quality = 1
            actual_holding_time = traffic_item.CalcRoute(solution, RouteCalcType.MinCostFlow, MAX_ROUTE, node_size,
                                                         current_link_list2, current_available_link_list)
            if not actual_holding_time:
                print("Undefined route calculation type\n")
                exit(-1)
        ...
        if active_traffic.end_time <= time:
            ...
            if active_traffic.traffic.total_data >= active_traffic.traffic.bandwidth * 1 * active_traffic.actual_holding_time:
                ...
                write_log3("[Achieve Total Data Failed(%d)] %d->%d (%d, %f, %d) %d\n" % (
                    ...
                    active_traffic.traffic.holding_time,
                    total_bandwidth))
            #print(total_data_transmitted3)
            #print(active_traffic.traffic.total_data)
            #print(active_traffic.traffic.bandwidth * 1 * active_traffic.actual_holding_time)
    ...
    if len(traffic_list) > 0:
        for traffic_item in traffic_list[0]:
            ...
            # ルート計算
            traffic_item.quality = 1
            actual_holding_time = traffic_item.CalcRoute(solution, RouteCalcType.Backup, 2, node_size,
                                                         current_link_list3, current_available_link_list)
            if not actual_holding_time:
                print("Undefined route calculation type\n")
                exit(-1)
            ...
            if solution.isOptimized():
                ...
                total_requested_expected_bandwidth3 += traffic_item.bandwidth * 1
                total_data_requested3 += traffic_item.bandwidth * 1 * traffic_item.holding_time
    ...
    #     expected_bandwidth = active_traffic_item.traffic.bandwidth * 1  # 帯域幅期待値
    #     total_bandwidth = 0
    #     for route in active_traffic_item.routes[:]:
    #         for route_link_key, route_link_item in route.items():
    #             total_bandwidth += route_link_item.bandwidth
    ...
    #         for ri in range(12):
    #             route_num = ri + 3
    #             actual_holding_time = traffic_item.CalcRoute(solution, RouteCalcType.KshortestExpectedCapacityGuarantee,
    #                                                          route_num, node_size, current_link_list4,
    #                                                          current_available_link_list, ks=10)
    #             if not actual_holding_time:
    #                 print("Undefined route calculation type\n")
    #                 exit(-1)
    #             if solution.isOptimized():
    ...
    #                 except:
    #                     break
    #             total_requested_expected_bandwidth4 += traffic_item.bandwidth * traffic_item.holding_time * 1
    ...
    #                     active_traffic.routes.append(route)
    #                     total_expected_bandwidth4 += route_reliability * actual_holding_time * route_bandwidth
    ...
    # 使用可能容量加算
    for active_traffic_item in active_traffic_list5[:]:
        # expected_bandwidth = active_traffic_item.traffic.bandwidth * active_traffic_item.traffic.quality  # 帯域幅期待値
        expected_bandwidth = active_traffic_item.traffic.bandwidth * 1
        total_bandwidth = 0
        for route in active_traffic_item.routes[:]:
            for route_link_key, route_link_item in route.items():
                ...
                break
        active_traffic_item.traffic.total_data += total_bandwidth
        write_log5("[Total Data(%d)] %d->%d %d\n" % (
            active_traffic_item.traffic.id, active_traffic_item.traffic.start_node,
            active_traffic_item.traffic.end_node, active_traffic_item.traffic.total_data))

    # 回線使用終了判定(マルチパス5)
    for active_traffic in active_traffic_list5[:]:
        if active_traffic.end_time <= time:
            # 回線使用終了
            # expected_bandwidth = active_traffic.traffic.bandwidth * active_traffic.traffic.quality
            expected_bandwidth = active_traffic.traffic.bandwidth * 1
            ...
            for route in active_traffic.routes:
                ...
                for used_link_key, used_link_item in route.items():
                    if not added_total_bandwidth:
                        total_bandwidth += used_link_item.bandwidth
                        added_total_bandwidth = True
                    current_link_list5[(used_link_key[0], used_link_key[1])].bandwidth += used_link_item.bandwidth
                    write_log5("Link %d->%d add bandwidth: %d\n" % (
                        used_link_key[0], used_link_key[1], used_link_item.bandwidth))

            if total_bandwidth >= expected_bandwidth:
                request_achieved_demand5 += 1
                write_log5("[End(%d)] %d->%d (%d, %f), %d\n" % (
                    active_traffic.traffic.id, active_traffic.traffic.start_node,
                    active_traffic.traffic.end_node,
                    active_traffic.traffic.bandwidth, active_traffic.traffic.quality,
                    active_traffic.end_time))
            else:
                write_log5("[End with Bandwidth Lowering(%d)] %d->%d (%d, %f)->%d, %d\n" % (
                    active_traffic.traffic.id, active_traffic.traffic.start_node,
                    active_traffic.traffic.end_node,
                    active_traffic.traffic.bandwidth, active_traffic.traffic.quality, total_bandwidth,
                    active_traffic.end_time))
            #if active_traffic.traffic.total_data >= active_traffic.traffic.bandwidth * active_traffic.traffic.quality * active_traffic.actual_holding_time:
            if active_traffic.traffic.total_data >= active_traffic.traffic.bandwidth * 1 * active_traffic.actual_holding_time:
                total_data_achieved_demand5 += 1
            else:
                write_log5("[Achieve Total Data Failed(%d)] %d->%d (%d, %f, %d) %d\n" % (
                    active_traffic.traffic.id, active_traffic.traffic.start_node, active_traffic.traffic.end_node,
                    active_traffic.traffic.bandwidth, active_traffic.traffic.quality,
                    active_traffic.traffic.holding_time,
                    total_bandwidth))
            #print(total_data_transmitted5)
            #print(active_traffic.traffic.total_data)
            #print(active_traffic.traffic.bandwidth * 1 * active_traffic.actual_holding_time)
            total_data_transmitted5 += active_traffic.traffic.total_data
            active_traffic_list5.remove(active_traffic)
    write_log5(show_links(current_link_list5))
    if len(traffic_list) > 0:
        for traffic_item in traffic_list[0]:
            solution = Solution()
            current_available_link_list = []  # type: list[tuple[int, int]]
            K = range(1, 3 + 1) # 最大経路数は3に設定

            # ルート計算
            traffic_item.quality = 0.5
            actual_holding_time = traffic_item.CalcRoute(solution, RouteCalcType.Multipath, 3, node_size,
                                                         current_link_list5, current_available_link_list)  # マルチパス変更点
            if not actual_holding_time:
                print("Undefined route calculation type\n")
                exit(-1)
            ...
            if solution.isOptimized():
                ...
                for k in K:
                    if solution.variables["b"][k] != 0:
                        for (i, j) in current_available_link_list:
                            if solution.variables["y"][(k, i, j)] != 0:
                                routes[k - 1][(i, j)] = Link(current_link_list5[(i, j)].distance,
                                                             solution.variables["y"][(k, i, j)],
                                                             current_link_list5[(i, j)].failure_rate,
                                                             0)
                #total_requested_expected_bandwidth5 += traffic_item.bandwidth * traffic_item.quality
                #total_data_requested5 += traffic_item.bandwidth * traffic_item.quality * traffic_item.holding_time
                total_requested_expected_bandwidth5 += traffic_item.bandwidth * 1
                total_data_requested5 += traffic_item.bandwidth * 1 * traffic_item.holding_time
                write_log5("[Accepted(%d)] %d->%d (%d, %f) for %d  \n" % (
                    traffic_item.id, traffic_item.start_node, traffic_item.end_node,
                    traffic_item.bandwidth * traffic_item.holding_time,
                    traffic_item.quality, actual_holding_time))
                # ルート使用処理
                ...
                for route in routes:
                    ...
                    if len(route) > 0:
                        ...
                        for (i, j), link in route.items():
                            route_bandwidth = link.bandwidth
                            current_link_list5[(i, j)].bandwidth -= link.bandwidth
                            route_reliability *= current_link_list5[(i, j)].calculate_reliability(
                                actual_holding_time)
                            write_log5("Link %d->%d remove bandwidth: %d\n" % (i, j, link.bandwidth))
                        active_traffic.routes.append(route)
                        total_expected_bandwidth5 += route_reliability * route_bandwidth
                active_traffic_list5.append(active_traffic)
            else:
                # 最適解なし
                # print("Blocked5")
                write_log5(show_links_wR(current_link_list, actual_holding_time))
                write_log5("[Blocked(%d)] %d->%d (%d, %f)\n" % (
                    traffic_item.id, traffic_item.start_node, traffic_item.end_node, traffic_item.bandwidth,
                    traffic_item.quality))
                blocked_bandwidth5 += traffic_item.bandwidth
                blocked_demand5 += 1
    # マルチパス処理5完了
...
with open(RESULT_FILE, "a") as f:
    ...
    f.write("Multipath Provisioning\n")
    f.write(
        "Blocked demand:%d(%d%%)\nTotal bandwidth: %d\nBlocked bandwidth: %d(%d%%)\nBandwidth achieved demand: %d(%d%%)\nTotal expected bandwidth: %d\nTotal requested expected bandwidth: %d\nTotal data achieved demand: %d\nTotal data transmitted: %d\nTotal data requested: %d\n"
